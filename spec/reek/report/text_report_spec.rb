...
RSpec.describe Reek::Report::TextReport do
  ...
  context 'with a single empty source' do
    before do
      instance.add_examiner Reek::Examiner.new('').run
    end
  ...
  context 'with non smelly files' do
    before do
      instance.add_examiner(Reek::Examiner.new('def simple() puts "a" end').run)
      instance.add_examiner(Reek::Examiner.new('def simple() puts "a" end').run)
    end
  ...
  context 'with a couple of smells' do
    before do
      instance.add_examiner(Reek::Examiner.new('def simple(a) a[3] end').run)
      instance.add_examiner(Reek::Examiner.new('def simple(a) a[3] end').run)
    end
